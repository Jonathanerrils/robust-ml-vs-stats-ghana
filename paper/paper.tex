...
The Markov-switching exchange-rate literature provides useful context
for interpreting this paper's own Markov-switching results. Engel's
...
\textbf{Logistic, LASSO, elastic net.} Standard, LASSO ($\ell_1$), and
elastic net ($\ell_1$/$\ell_2$, mixing ratio 0.5) logistic regression,
...
\textbf{Random forest, XGBoost.} Standard ensemble tree methods.
...
Point-estimate AUC comparisons are tested for significance by a paired
...
Permutation-importance and SHAP diagnostics were recomputed on the
